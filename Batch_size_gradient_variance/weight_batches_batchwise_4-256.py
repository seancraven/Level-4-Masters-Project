...
for batchsize in [2**j for j in range(8,9)]:
    ...
    batches  = (weights_dict['w_1']).shape[2]
    
    rms_weight = np.zeros((batches,6))
    std_weight = np.zeros((batches,6))
    rms_wieght_delta = np.zeros((batches,6))
    std_wieght_delta = np.zeros((batches,6))
