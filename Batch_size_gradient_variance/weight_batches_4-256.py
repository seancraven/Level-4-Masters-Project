...
##Load Training Data
training_data_array = np.loadtxt('/home/sean/Documents/Work/Level 4/Level-4-Masters-Project/Batch_size_gradient_variance/Weights_Data/training_data.csv', delimiter = ',')
training_data = (training_data_array[:,:16],training_data_array[:,16])
        ...
        for layer_i in range(len(self.model.layers)):
            ...
            if epoch == 0:
                ...
                weights_dict['b_'+str(layer_i+1)] = np.dstack(
                    (weights_dict['b_'+str(layer_i+1)], b))
for batchsize in [2**j for j in range(4,10)]:
    ...
    # Train Network 
    history = model.fit(train_x,train_y,batch_size= batchsize , epochs = epochs, callbacks= GetWeights(),verbose= 0)
    min_mape = np.min(history.history['mean_absolute_percentage_error'])
    print('Best MAPE for network Batch Size {} is {}'.format(batchsize,min_mape))
    ...
    for j,_ in enumerate(weights_dict.items()):
        if j%2 or j ==0 :#Only Weights not Biases
    ...
    extention_w = 'weight/weight_batch_size_{}'.format(batchsize)
